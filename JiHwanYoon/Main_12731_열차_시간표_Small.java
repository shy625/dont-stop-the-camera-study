...
public class Main_12731_열차_시간표_Small {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int n = 1; n <= N; n++) {
			...
			// 열차역 A, B에 계획된 열차의 출발 시각과 도착 시각을 저장하는 우선순위 큐
			// 출발 시각이 빠른 순서대로 저장
			PriorityQueue<Train> plansA = new PriorityQueue<>();
			PriorityQueue<Train> plansB = new PriorityQueue<>();
			// 입력을 받는 과정이 열차역 A, B가 동일해 함수로 처리
			putTimesToPlans(plansA, NA);
			putTimesToPlans(plansB, NB);
			// 출발이 가장 빠른 열차의 출발 시각을 나타낸다.
			// 만약 열차역에 출발할 열차가 남아있지 않으면 Integer.MAX_VALUE를 가지도록 한다.
			int curTimeA = Integer.MAX_VALUE;
			int curTimeB = Integer.MAX_VALUE;
			// 특정 열차역에서 열차를 출발시키면 다른 열차역에 열차가 도착하게 되는데, 
			// 해당 열차는 나중에 다른 운행 계획을 만족시킬 수도 있기 때문에 이를 각 열차역에 대해 저장해둔다.
			// 이때 정렬 기준은 도착 시각이 빠른 순서대로 정렬해 존재하는 열차를 최대한 활용한다.
			...
			// 각 열차역에서 첫차로 출발시켜야 하는 열차 수
			int cntA = 0;
			int cntB = 0;
			// 모든 운행 계획을 성사시킬 때까지 진행
			while (!plansA.isEmpty() || !plansB.isEmpty()) {
				// 각 열차역에서 운행 계획 중 출발이 가장 빠른 열차의 출발 시각을 찾는다.
				...
				// 만약 열차역 A에서 더 빠른 운행 계획이 있으면 해당 열차를 B로 운행한다.
				if (curTimeA <= curTimeB) {
					Train t = plansA.poll();
					cntA = moveTrain(t, waitsA, waitsB, cntA);
				// 열차역 B에서 더 빠른 운행 계획이 있으면 해당 열차를 A로 운행한다.
				} else {
					Train t = plansB.poll();
	...
	// 열차 t를 1번 역에서 2번 역까지 운행하는 함수
	// waits1은 1번 역에서 대기 중인 열차들, waits2는 2번 역에서 대기 중인 열차들, 
	// cnt는 1번 역에서 첫차로 출발해야 하는 열차의 개수를 의미한다.
	private static int moveTrain(Train t, PriorityQueue<Train> waits1, PriorityQueue<Train> waits2, int cnt) {
		// 1번 역에 2번 역으로부터 온 열차가 남아있고, 회차시간을 고려했을 때 현재 출발할 수 있는 경우
		if (!waits1.isEmpty() && waits1.peek().endTime+T <= t.startTime) waits1.poll();
		// 그렇지 않은 경우 새로운 열차를 첫차로 출발시킨다.
		else cnt++;
		// 열차 t는 2번 역에 도착한다.
		waits2.offer(t);
		return cnt;
